...
test("Não pode iniciar uma corrida se ela não foi aceita", function(){
    const ride = Ride.create("", new Coord(0, 0), new Coord(0, 0));   
    expect(()=> ride.start(new Date("2021-03-01T10:40:00"))).toThrow("A corrida não foi aceita");
});


test("Não pode finalizar uma corrida se ela não foi iniciada", function(){
    const ride = Ride.create("", new Coord(0, 0), new Coord(0, 0));   
    expect(()=> ride.end(new Date("2021-03-01T10:40:00"))).toThrow("A corrida não foi iniciada");
});
