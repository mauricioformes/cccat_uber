...
test("Deve lançar um erro se a data for inválida", function () {
    const ride = Ride.create("", new Coord(0,0), new Coord(0,0));
    ride.addPosition(-27.584905257808835, -48.545022195325124, new Date("umastringnolugardadata"));
    ride.addPosition(-27.496887588317275, -48.522234807851476, new Date("umastringnolugardadata"));
    expect(() => ride.calculate()).toThrow(new Error("Data inválida"));
});
...
test("Deve finalizar uma corrida", function(){
    ...
    expect(ride.status.value).toBe("finished");
});

test("Não pode iniciar uma corrida se ela não foi aceita", function(){
    const ride = Ride.create("", new Coord(0, 0), new Coord(0, 0));   
    expect(()=> ride.start(new Date("2021-03-01T10:40:00"))).toThrow("Status inválido");
});


test("Não pode finalizar uma corrida se ela não foi iniciada", function(){
    const ride = Ride.create("", new Coord(0, 0), new Coord(0, 0));   
    expect(()=> ride.end(new Date("2021-03-01T10:40:00"))).toThrow("Status inválido");
});
