import Passenger from "../../src/domain/passageiro/Passenger";
...
test("Não deve criar um passageiro com CPF inválido", function () {
    expect(() => Passenger.create("John Doe", "john.doe@gmail.com", "83432616076")).toThrow(new Error("CPF Inválido"));
});

test("Não deve criar um passageiro com email inválido", function () {
    expect(() => Passenger.create("John Doe", "john.doe@gmail", "83432616074")).toThrow(new Error("Email Inválido"));
});