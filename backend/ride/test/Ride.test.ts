...
test("Deve retornar -1 se a distância for inválida", function () {
    const ride = new Ride();
    expect(() => ride.addSegment(-10, new Date("2023-03-01T10:00:00"))).toThrow(new Error("Distância inválida"));
});

test("Deve retornar -2 se a data for inválida", function () {
    const ride = new Ride();
    expect(() => ride.addSegment(10, new Date("javascript"))).toThrow(new Error("Data inválida"));
});
