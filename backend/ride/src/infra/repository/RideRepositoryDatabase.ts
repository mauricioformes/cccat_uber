...
export default class RideRepositoryDatabase implements RideRepository {
    ...
    async get(rideId: string): Promise<Ride> {
        ...
        const ride = new Ride(rideData.ride_id, rideData.passenger_id, new Coord(parseFloat(rideData.from_lat), parseFloat(rideData.from_long)),
            new Coord(parseFloat(rideData.to_lat), parseFloat(rideData.to_long)), rideData.status, rideData.request_date);
